...
const GalleryPage = () => {
  ...
  const galleryImages = [
    microscopeAnalysis, labEquipmentSoxhlet, colorfulSamples, gcmsInstrument,
    chemicalFlasks, labAnalysisScreen, blueChemicalTest, touchscreenAnalysis,
    sampleVialsTray, advancedGcmsSystem, icapSpectrometer, discreteAnalyzer,
    labWorkstation, behrotestEquipment, hachAnalyzer, microbiologicalInspection,
    acolyteDevice, incubatorCultures, memmertIncubator, nikonMicroscope
  ];
                ...
                <ScrollAnimation key={index} delay={0.05 * index}>
                  <Dialog>
                    <DialogTrigger asChild>
                      <Card className="overflow-hidden shadow-elegant hover:shadow-glow transition-all duration-500 group cursor-pointer border-0">
                        <div className="relative overflow-hidden aspect-square">
                          <img src={image} alt={`Laboratory image ${index + 1}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    ...
                    <DialogContent className="max-w-5xl p-0 bg-transparent border-0">
                      <img src={image} alt={`Laboratory image ${index + 1}`} className="w-full h-auto rounded-lg" />
                    </DialogContent>
                  </Dialog>
                </ScrollAnimation>
              ))}
