...
const Services = () => {
  ...
  return <section id="services" className="py-20 surface-gradient">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {services.map((service, index) => <Card key={index} className="group card-gradient shadow-elegant hover:shadow-glow transition-spring cursor-pointer overflow-hidden">
              {/* Image/Icon Header */}
              <div className="relative h-48 bg-gradient-to-br from-primary/10 to-accent/10 flex items-center justify-center overflow-hidden">
                {service.image ? <img src={service.image} alt={service.title} className="w-full h-full object-cover group-hover:scale-110 transition-spring" /> : <service.icon className="w-16 h-16 text-primary group-hover:scale-110 group-hover:text-accent transition-spring" />}
