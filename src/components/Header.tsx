      ...
      <div className="w-full px-4 lg:px-8 relative z-10">
        <div className="flex items-center justify-between h-20 lg:h-24">
          ...
          <nav className={`hidden lg:flex items-center whitespace-nowrap ${isRTL ? "space-x-reverse space-x-6" : "space-x-6"}`}>
            ...
            if (item.subItems) {
              return <div key={item.labelKey} className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                    ...
                    {isAboutOpen && <>
                        ...
                        <div className={`absolute top-full ${isRTL ? "right-0" : "left-0"} pt-2 z-50`}>
                          <div className="bg-white border border-border rounded-lg shadow-lg py-2 min-w-[160px]">
                            <Link to={item.href} onClick={() => setIsAboutOpen(false)} className="block px-4 py-2 text-sm text-foreground hover:bg-muted hover:text-primary transition-colors">
                              {t("nav.aboutUs")}
                            </Link>
                            {item.subItems.map(subItem => <Link key={subItem.labelKey} to={subItem.href} onClick={() => setIsAboutOpen(false)} className={`block px-4 py-2 text-sm hover:bg-muted transition-colors ${location.pathname === subItem.href ? "text-blue-600" : "text-foreground hover:text-primary"}`}>
                                {t(subItem.labelKey)}
                              </Link>)}
                          </div>
                        </div>
            ...
            return <Link key={item.labelKey} to={item.href} className={`text-base font-medium transition-smooth relative group whitespace-nowrap ${isActive ? "text-blue-600 hover:text-blue-800" : "text-foreground hover:text-primary"}`}>
                  {t(item.labelKey)}
        ...
        {isMenuOpen && <div className="lg:hidden absolute top-full left-0 right-0 bg-background/95 backdrop-blur-sm border-b shadow-medium animate-fade-up">
            <nav className="container mx-auto px-4 py-4">
              ...
              const isActive = location.pathname === item.href || item.subItems?.some(sub => location.pathname === sub.href);
              if (item.subItems) {
                return <div key={item.labelKey} className="flex flex-col">
                        <button className={`transition-smooth py-2 flex items-center justify-between ${isActive ? "text-blue-600 hover:text-blue-800" : "text-foreground hover:text-primary"}`} onClick={() => setIsMobileAboutOpen(!isMobileAboutOpen)}>
                          {t(item.labelKey)}
                          <ChevronDown className={`w-4 h-4 transition-transform ${isMobileAboutOpen ? "rotate-180" : ""}`} />
                        </button>
                        {isMobileAboutOpen && <div className={`${isRTL ? "pr-4 mr-2 border-r-2" : "pl-4 ml-2 border-l-2"} flex flex-col border-muted`}>
                            <Link to={item.href} className="transition-smooth py-2 text-sm text-foreground hover:text-primary" onClick={() => setIsMenuOpen(false)}>
                              {t("nav.aboutUs")}
                            </Link>
                            {item.subItems.map(subItem => <Link key={subItem.labelKey} to={subItem.href} className={`transition-smooth py-2 text-sm ${location.pathname === subItem.href ? "text-blue-600" : "text-foreground hover:text-primary"}`} onClick={() => setIsMenuOpen(false)}>
                                {t(subItem.labelKey)}
                              </Link>)}
                          </div>}
                      </div>;
              }
              return <Link key={item.labelKey} to={item.href} className={`transition-smooth py-2 ${isActive ? "text-blue-600 hover:text-blue-800" : "text-foreground hover:text-primary"}`} onClick={() => setIsMenuOpen(false)}>
                      {t(item.labelKey)}
                    </Link>;
            })}
